...
class BinaryTree:
    ...
    def inorden_superheroes_descendentes(self):
        def __inorden_superheroes_descendentes(root):
         if root is not None:
            __inorden_superheroes_descendentes(root.right)  # Visitar el subárbol derecho primero para obtener orden descendente
            if root.other_value.get('is_hero') is True:
                print(root.value)  # Imprimir solo los superhéroes
                __inorden_superheroes_descendentes(root.left)  # Luego visitar el subárbol izquierdo

        if self.root is not None:
            __inorden_superheroes_descendentes(self.root)
